...
data_with_prio = genfromtxt('output/data_with_prio', delimiter=';')
...
axs[0, 0].hist(data_node, range=(0,100), bins=100)
axs[0, 0].set_title('Node WASM')

axs[0, 1].hist(data_no_prio, range=(0,100), bins=100)
axs[0, 1].set_title('C (no prio)')

axs[0, 2].hist(data_with_prio, range=(0,100), bins=100)
axs[0, 2].set_title('C (with prio)')

axs[1, 0].hist(data_wasmtime, range=(0,100), bins=100)
...
plt.close()
plt.boxplot([data_node, data_no_prio, data_with_prio, data_wasmtime, data_wasmer], labels=['Node WASM', 'C (no prio)', 'C (with prio)', 'wasmtime', 'wasmer'])
